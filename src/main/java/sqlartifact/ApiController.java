...
public class ApiController {
	...
	public String Abc() {
		...
		try {
			...
			while (result.next()) {
				res.add("user_id", result.getString("user_id"));
				res.add("username", result.getString("username"));
			}
			con.close();
	...
	@ResponseBody
	public String Create(@RequestParam("name") String name, @RequestParam("subject1") String subject1,
...
//	        	 return "{status: true, message: success, batchId: "+batchMapped+"}";
			...
			if (result != null)
				res = Json.createObjectBuilder().add("status", true).add("message", "success").add("name", nameJson)
						.add("id", idJson).add("subject1", subject1Json).add("subject2", subject2Json)
						.add("subject3", subject3Json).add("average", averageJson);
//	         else {
//	        	 con.close();
...
//	         pstmt.setInt(1,Integer.parseInt(id));
	...
	public String getById(@PathVariable("id") String id) {
		...
		try {
			...
			if (result.next()) {
				res = Json.createObjectBuilder().add("status", true).add("message", "success")
						.add("name", result.getString("name")).add("subject1", result.getString("subject1"))
						.add("subject2", result.getString("subject2")).add("subject3", result.getString("subject3"));
				con.close();
			} else {
				con.close();
				res = Json.createObjectBuilder().add("status", true).add("message",
						"no data found against corresponding ID");
	...
	public String update(@PathVariable("id") String id, @RequestParam("name") String name,
			@RequestParam("subject1") String subject1, @RequestParam("subject2") String subject2,
	...
	@ResponseBody
	public String operations(@RequestParam("function") String function, @RequestParam("subject") String subject) {
		// System.out.println(userId);
		DataSource ds;
		Connection con;
		JsonObjectBuilder res = Json.createObjectBuilder();
...
//	         System.out.println();
			...
			if (result.next()) {
				if (function.equals("avg")) {
					res = Json.createObjectBuilder().add("status", true).add("message", "success").add("value",
	...
	@RequestMapping(value = "/five/{select}", produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public String TopFive(@PathVariable("select") String select) {
		...
		try {
			...
			PreparedStatement pstmt = null;
			String query = null;
			query = "SELECT * FROM `user_details` ORDER BY `subject1` " + select + ", `subject2` " + select
					+ ", `subject3`" + select + " LIMIT 5;";
			pstmt = con.prepareStatement(query);
			ResultSet result = pstmt.executeQuery();
			System.out.println(result);
			ArrayList<String> name = new ArrayList<String>();
			...
			if (result != null)
				res = Json.createObjectBuilder().add("status", true).add("message", "success").add("name", nameJson)
						.add("subject1", subject1Json).add("subject2", subject2Json).add("subject3", subject3Json);
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
		JsonObject jsonObject = res.build();
	...
	@ResponseBody
	public String Filters(@RequestParam("filter") String filter, @RequestParam("subject") String subject,
